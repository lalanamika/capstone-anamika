...
extra_adjectives_file = path+'/extra_adjectives_list.txt'
extra_words_list_file = path+'/extra_words_list.txt'
...
extra_adjectives = set(line.strip() for line in open(extra_adjectives_file))

# Remove some extra words like 'assorted', 'approximately' etc. QUESTION: Is there a smart way to remove the top 100 such words?
extra_words = set(line.strip() for line in open(extra_words_list_file))
